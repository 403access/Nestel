...
export const useQuizGame = (): UseQuizGame => {
  const [currentProduct, setCurrentProduct] = useState<BreadProduct | null>(
    null
  )
  ...
  const checkAnswer = useCallback(() => {
    ...
    if (
      correctCount === totalPossibleCorrect &&
      totalSelected === totalPossibleCorrect
    ) {
      setScore((prev) => prev + 1)
      Alert.alert("Correct!", "You guessed all ingredients correctly!", [
        { text: "Next Question", onPress: loadNewQuestion }
      ])
    } else {
      let message = "Incorrect! You missed some or selected extra ingredients."
      const missing = Array.from(correctIngredients)
        .filter((id) => !selectedIngredients.has(id))
        .map((id) => ALL_INGREDIENTS.find((ing) => ing.id === id)?.name)
      const extra = Array.from(selectedIngredients)
        .filter((id) => !correctIngredients.has(id))
        .map((id) => ALL_INGREDIENTS.find((ing) => ing.id === id)?.name)
