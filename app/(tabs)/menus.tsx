...
interface MenuOfferCardProps {
  offer: { id: number; name: string; items: { productId: number; discountPrice?: number }[] };
  menu: Menu;
}
...
const MenuOfferCard: React.FC<MenuOfferCardProps> = ({ offer, menu }) => {
  return (
    <View style={offerStyles.offerCard}>
      ...
      {offer.description && <Text style={offerStyles.offerDescription}>{offer.description}</Text>}
            ...
            <View style={offerStyles.productDetails}>
              ...
              <View style={offerStyles.priceContainer}>
                {originalPrice > currentPrice && (
                  <Text style={offerStyles.originalPriceStrikethrough}>€{originalPrice.toFixed(2)}</Text>
                )}
                <Text style={offerStyles.currentPrice}>€{currentPrice.toFixed(2)}</Text>
              </View>
            </View>
          </Link>
        );
...
const MenuCard: React.FC<MenuCardProps> = ({ menu }) => {
  return (
    <View style={menuStyles.menuCard}>
      ...
      {menu.description && <Text style={menuStyles.menuDescription}>{menu.description}</Text>}
      {menu.offers && menu.offers.map(offer => (
        <MenuOfferCard key={offer.id} offer={offer} menu={menu} />
      ))}
      {!menu.offers?.length && <Text style={menuStyles.noOffersText}>No offers available for this menu.</Text>}
    </View>
  );
};
